...
export class LecturereportService {
	...
	async create(dto: CreateLectureReportDto, images: Express.Multer.File[]) {
		...
		if (!lecture) {
			throw new BadRequestException(
				'Программа обучения с такой ID не существует'
			)
		}

		if (lecture.status == 'CLOSED') {
			throw new BadRequestException(
				'Программа обучения с такой ID завершён и более недоступен'
			)
		}

		if (lecture.status == 'WAITING') {
			throw new BadRequestException(
				'Программа обучения с такой ID приостановлен и будет закрыт'
			)
		}
